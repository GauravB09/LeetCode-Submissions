...
class Solution:
    def partition(self, head: Optional[ListNode], x: int) -> Optional[ListNode]:
        ...
        # return result_head
        dummy_lesser_head = dummy_lesser = ListNode()
        dummy_greater_head = dummy_greater = ListNode()
        while head:
            temp = head.next
            if head.val < x:
                dummy_lesser.next = head
                head.next = None
                dummy_lesser = dummy_lesser.next
            else:
                dummy_greater.next = head
                head.next = None
                dummy_greater = dummy_greater.next
            head = temp
        dummy_lesser.next = dummy_greater_head.next
        return dummy_lesser_head.next
